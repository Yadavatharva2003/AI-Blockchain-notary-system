    ...
    if (user) {
      ...
      const unsubscribe = onSnapshot(q, (snapshot) => {
        const docs = snapshot.docs.map((doc) => ({
          ...
          uploadDate: new Date(
            doc.data().uploadTime.toDate()
          ).toLocaleDateString(),
          uploadTime: doc.data().uploadTime, // Store the original Firestore Timestamp
  ...
  const filteredDocuments = documents
    ...
    .sort((a, b) => {
      if (sortBy === "fileName") {
        ...
        return b.uploadTime.toMillis() - a.uploadTime.toMillis();
  ...
  const handleRevokeNotarization = async (documentId) => {
    try {
      ...
      setDocuments((prevDocs) =>
        prevDocs.map((doc) =>
          doc.id === documentId ? { ...doc, status: "Revoked" } : doc
        )
      );
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10">
        <div className="pt-12 pb-6 px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <div className="mb-4 flex flex-col sm:flex-row items-center justify-between">
              <div className="flex items-center mb-2 sm:mb-0">
                <input
                  ...
                  className={`border border-gray-300 rounded-lg px-4 py-2 transition-transform duration-300 ${
            ...
            {/* Header Row */}
            <div
              className={`grid grid-cols-1 sm:grid-cols-5 gap-6 font-bold ${
                darkMode
            ...
            >
              <div
                className="header-cell py-2 pl-4 border-l-4 border-blue-500"
                style={{ minWidth: "250px" }}
              >
                File Name
              </div>{" "}
              {/* Increased width */}
              <div className="header-cell py-2 text-center">Status</div>{" "}
              {/* Centered text */}
              <div className="header-cell py-2 text-center">
                Upload Date
              </div>{" "}
              {/* Centered text */}
              <div className="header-cell py-2 text-center">
              ...
              {/* Centered text */}
              <div className="header-cell py-2 text-center">Action</div>{" "}
              {/* Centered text */}
            </div>
            ...
            <div className="flex flex-col space-y-4 mt-4">
              {filteredDocuments.map((doc, index) => (
                <motion.div
                  key={doc.id}
                  className={`border rounded-lg p-4 transition-transform duration-300 ${
                    darkMode ? "bg-gray-800" : "bg-white"
                  } cursor-pointer w-full hover:scale-105`} // Added zoom effect
                ...
                >
                  <div className="grid grid-cols-1 sm:grid-cols-5 gap-6 items-center">
                    <div
                      className="data-cell text-lg break-words"
                      style={{ minWidth: "250px" }}
                    >
                      {doc.fileName}
                    </div>{" "}
                    {/* Increased width */}
                    <div
                      className={`data-cell status text-center ${
                        doc.status === "Verified"
                          ? "text-green-500"
                          : doc.status === "Rejected"
                          ? "text-red-500"
                      ...
                      {doc.details && (
                        <div className="space-y-2">
                          ...
                          {doc.details.notaryStatus && (
                            <div
                              className={
                                doc.details.notaryStatus.isValid
                                  ? "text-green-500"
                                  : "text-red-500"
                              }
                            >
                              ...
                              <ul className="list-disc ml-4">
                                {Object.entries(
                                  doc.details.notaryStatus.checks
                                ).map(([key, value]) => (
                                  <li key={key}>
                          ...
                          {doc.details.criticalErrors &&
                            doc.details.criticalErrors.length > 0 && (
                              <div className="text-red-500">
                                <p className="font-semibold">
                                  Critical Errors:
                                </p>
                                <ul className="list-disc ml-4">
                                  {doc.details.criticalErrors.map(
                                    (error, i) => (
                                      <li key={i}>{error}</li>
                                    )
                                  )}
                          ...
                          {doc.details.spellingErrors &&
                            doc.details.spellingErrors.length > 0 && (
                              <div className="text-yellow-500">
                                <p className="font-semibold">
                                  Spelling Issues:
                                </p>
                                <ul className="list-disc ml-4">
                                  {doc.details.spellingErrors.map(
                                    (error, i) => (
                                      <li key={i}>{error}</li>
                                    )
                                  )}
                          ...
                          {doc.details.formatErrors &&
                            doc.details.formatErrors.length > 0 && (
      ...
      <Transition appear show={showRevokeConfirmation} as={Fragment}>
        <Dialog as="div" className="relative z-50" onClose={cancelRevoke}>
          <Transition.Child
            as={Fragment}
