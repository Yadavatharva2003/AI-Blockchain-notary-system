...
function detectSpellingErrors(text) {
  ...
  for (const word of words) {
    ...
    if (
      /[aeiou]{4,}|[bcdfghjklmnpqrstvwxyz]{5,}/.test(cleanWord) ||
...
function detectFormatErrors(text) {
  ...
  const spacingErrors = text.match(
    errorPatterns.formatErrors.inconsistentSpacing
  );
  if (spacingErrors) {
...
async function verifyDocument(documentContent) {
  try {
    const documentAnalysis = await checkForLegalDocumentCharacteristics(
      documentContent
    );
    ...
    const warnings = [];
    if (documentAnalysis.spellingErrors)
      warnings.push("Potential spelling issues found");
    if (documentAnalysis.formatErrors)
      warnings.push("Minor format issues detected");
